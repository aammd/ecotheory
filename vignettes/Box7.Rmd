...
8. `final.rad` is the rank-order relative abundance vector at the final time step.  This needs to be saved if we want to use it as the metacommunity relative abundance distribution in Online Box 8.  Note that you will need to create/set the directory on your own (i.e., replaced `c:/temp/RAD1.csv` with whatever directory and filename  you want). 
