...
8. After each sequence of `J` deaths, record data. `i %% J` returns the remainder of `i` divided by `J`, and each time that `J` deaths have occurred this will be equal to zero, so this is an efficient way to tell R to “stop” the program, record the frequency of species 1, and increment the year tracker by 1.  Note that the `}` after this loop closes the `for` loop from step 5.
